import { BookOpenText, CheckCircle2, Code2, FileText, Megaphone, Network } from 'lucide-react';
import ResourcePageShell from '@/components/public/resource-page-shell';
import { Badge } from '@/components/ui/badge';
import { PROJECT_OUTPUTS } from '@/lib/constants';

export const metadata = {
  title: 'Luaran Proyek 3 - Kavana',
  description: 'Representasi luaran Proyek 3 untuk aplikasi Kavana.',
};

const OUTPUT_ICONS = [Network, FileText, BookOpenText, Megaphone];

export default function LuaranProyekTigaPage() {
  const project = PROJECT_OUTPUTS.project3;

  return (
    <ResourcePageShell
      title={project.title}
      subtitle={project.summary}
      updatedLabel="9 Mei 2026"
    >
      <article>
        ...
        <h2 className="mt-2 text-2xl font-black tracking-tight text-[hsl(var(--ctp-text))]">
          Luaran Proyek 3 menekankan web services dengan dokumentasi API yang bisa diuji dan dipahami.
        </h2>
        <p className="mt-4 max-w-3xl text-sm leading-8 text-[hsl(var(--ctp-subtext1))]">
          Paket luaran ini memosisikan aplikasi web services sebagai bukti implementasi teknis utama,
          sementara draft jurnal, buku tutorial, dan poster menjadi artefak akademik untuk menjelaskan
          rancangan, cara penggunaan, hasil pengujian, dan nilai layanan.
        </p>
      </article>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-[28px] border border-[hsl(var(--ctp-surface1))] bg-[hsl(var(--ctp-base)/0.76)] p-6">
          <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[hsl(var(--ctp-subtext0))]">
